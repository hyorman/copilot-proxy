...
import { state, PendingToolContext } from './state.js';
import { ChatBackend } from '../backend.js';
import { ChatCompletionRequest, ChatCompletionChunk, ChatCompletionResponse, ChatMessage } from '../types.js';
...
export function setRunnerBackend(b: ChatBackend): void {
  backend = b;
}
...
export async function* executeRun(
  ...
  try {
    ...
    if (streaming) {
      ...
      const streamIterator = await getBackend().processChatRequest(request) as AsyncIterable<ChatCompletionChunk>;
      ...
      // Non-streaming mode
      const response = await getBackend().processChatRequest(request) as ChatCompletionResponse;
